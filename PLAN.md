...
**解決策:** SAM 3のビデオトラッキング機能を使用して、複数フレームの点群を自動統合

#### SAM 3 ビデオトラッキング

SAM 3はSAM 2のトラッキングアーキテクチャを継承しており、さらにテキストプロンプトでの追跡もサポートしている。
...
# セッション開始（MP4ファイルまたはJPEGフォルダを指定）
video_path = "recording/video.mp4"

# プロンプト追加（クリック or テキスト）
# - frame_index: 対象フレーム
# - text: "yellow chair" などのテキスト記述
# - または point_coords でクリック指定
```
...
| 設定 | 推奨値 | 備考 |
|------|--------|------|
| Camera | `.abc` (Alembic) | オープン形式、Pythonでパース可能 |
| Third Party | Blender | 座標系が明確（Z-up） |
...
│   ├── YYYYMMDD_HHMMSS_depth_00001.png
│   └── ...
...
```json
{
  "data": {
    "video": {
      "relative_path": "./003.mov",
      "fps": 60,
      "resolution": {"width": 1080, "height": 1920}
    },
    "geometry": {
      "relative_path": ["./scan_*.obj"]
    },
    "camera": {
      "frames": [
        {
...
┌─────────────────────────────────────────────────────────────┐
│ Omniscientで撮影                                            │
│   動画(MOV) + 深度(PNG) + カメラポーズ(ABC)                 │
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
│ フレーム0でクリック or テキストセグメント                   │
│   SAM 3でオブジェクトを指定（例: "椅子"）                   │
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
│ SAM 3がフレーム1〜Nにマスク自動伝播                         │
│   ビデオトラッキング機能（SAM 2アーキテクチャ継承）         │
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
│ 各フレームの点群を抽出                                      │
│   マスク × 深度(16bit PNG) × カメラパラメータ → 3D点群     │
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
│ カメラポーズで位置合わせ・統合                              │
│   Alembic(.abc)の高精度ポーズを使用（ICP不要）              │
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
│ 高密度統合点群 → SAM 3Dと融合                               │
